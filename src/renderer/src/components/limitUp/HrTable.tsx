    ...
    <div className="flex h-full flex-col">
      ...
      {(topReasons.length > 0 ||
        ...
        <Card className="mb-3" size="small" title="涨停原因高频词统计">
          <Collapse ghost defaultActiveKey={['all', 'star', 'gem']}>
            {/* 全部市场 */}
            {topReasons.length > 0 && (
              <Panel header={`全部市场 (${totalCount}支)`} key="all">
                <Space wrap>
            ...
            {/* 科创板 */}
            {starReasons.length > 0 && (
              <Panel header={`科创板 (${starCount}支)`} key="star">
                <Space wrap>
            ...
            {/* 创业板 */}
            {gemReasons.length > 0 && (
              <Panel header={`创业板 (${gemCount}支)`} key="gem">
                <Space wrap>
