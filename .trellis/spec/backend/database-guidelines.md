...
- Feishu stock-detail reasons prefer non-empty `limit_up_pool` values and fall back per field to the first non-empty `block_top_stock.reason_type` / `reason_info` for the same stock code, so reason details present only in 最强风口 data remain visible.
...
- The prompt must wrap strategy text in a path-free internal-methodology tag and forbid mentioning the methodology, document source, filename, path, or original content in model output. Every review conclusion must cite daily factual material; strategy-dependent fields absent from that material remain `数据不足` or `盘中/竞价待观察`.
  - Market-state output separates `高潮` from `强分歧`, starts with a previous-day-to-current-day transition, and emits `状态迁移无法判断` when prior facts are unavailable.
  - Structural expectation uses only paired prior/current facts and labels the result `超预期`, `符合预期`, `不及预期`, or `无法判断`; it never invents auction, gap, volume, or intraday baselines.
  - Board evidence uses member overlap to avoid counting the same stocks as independent themes. Leader-role transitions such as catch-up, succession, transition, or recovery remain candidates unless chronology, relative height, board strength, and old-leader feedback are all supported.
  - Main-board, GEM/STAR, and ST space systems are not mixed or inferred from code/name when explicit market-type facts are absent. Unsupported methodology chapters remain inapplicable rather than becoming same-day claims.
...
### 6. Tests Required
- Use a fake Codex client to assert environment-driven strategy loading, complete strategy inclusion, path-free prompt content, state-transition and expectation labels, board-overlap deduplication, role-transition boundaries, strategy-path metadata, shared factual input, both THS reason fields, one-call caching, and forced replacement without a live model call.
...
- Feishu tests assert score/suggestion sections are absent, persisted review/provider/model are appended, chart and table element IDs are unique, hot-block tables include every member stock with turnover and Eastmoney industry representative stocks use one newline-delimited entry per stock in table cells, the continuous-ladder and high-turnover high-board tables expose both concise `reason_type` and full `reason_info`, a paginated high-turnover high-board table lists only `continuous_days >= 3` stocks whose turnover is strictly above 10%, and board/industry charts keep every item instead of a Top-N slice.
